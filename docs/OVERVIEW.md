...
[driver]
  flow（source→executor を束ねるループ。SessionStart で /flow を提案）
      ↓ source を引く（compass の一手 / backlog のキュー）
[上流]
  compass（目標再接地）／ backlog（確定キュー）
      ↓ 一手の課題文
  condukt（実行の背骨）
...
|---|---|---|
| SessionStart | セッション開始時 | ctxrot(restore), condukt(restore), compass, daily, difflog, flow(propose), specguard, taskprog |
