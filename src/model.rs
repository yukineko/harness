...
//! deserializes — when `donegate gate` is run by hand (no stdin JSON) we fall
//! back to a default and behave as a plain CLI checker.
...
pub struct HookInput {
    ...
    /// attempt counter, but keep this for completeness.
    #[serde(default)]
...
impl HookInput {
    ...
    /// A stable session key for the attempt counter. Empty session ids (manual
