//! serde struct for the Claude Code **Stop** and **Notification** hook stdin
//! payloads. Every field is optional so an empty/foreign payload still
//! deserializes; a manual run with no stdin JSON is a no-op.
...
pub struct HookInput {
    ...
    /// Path to the JSONL transcript (used to extract a tail snippet on Stop).
    #[serde(default)]
