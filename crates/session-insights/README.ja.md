...
- **SessionEnd フック**（`stop` と `sessionend`）がターンを 1 つ数え、任意で Obsidian の日付つきセッションノートを書き出す。
...
同梱の `hooks/hooks.json` が両フックを自動で配線する。PostToolUse が `session-insights record` を、SessionEnd が `session-insights stop` と `session-insights sessionend` を呼ぶ。閾値の調整や Obsidian ログの有効化は `session-insights.toml` を置いて行う。
