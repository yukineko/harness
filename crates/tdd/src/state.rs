...
//! an endless write-a-test→stop→block loop.
...
/// count. Treated as 0 if more than `reset_after_secs` elapsed since the last
/// block (a fresh turn).
...
/// Clear the counter (a clean stop with no blocking finding).
