...
| `ctxrot toolguard` | `PostToolUse` | `Read`/`Bash`/`Grep`/… が巨大なペイロードを返した時、*次の*重い読みを sub-agent 経由にして結論だけ残すよう促す（preguard ゲートを通り抜ける 50KB〜1MB の中間帯を扱う）。 |
...
推奨は **Claude Code プラグインとしての導入**。このリポジトリは Rust crate であると同時にプラグイン／マーケットプレースでもあり、5 つのフック・`/distill` と `/ctx` スキル・`ctxrot-distiller` subagent・ビルド済みバイナリ（`bin/ctxrot`）を同梱する。すべて Claude **サブスクリプション**で完結し、API キーも別途の `cargo install` も不要。
