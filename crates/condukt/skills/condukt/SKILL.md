...
#### カスケードエスカレーション
verifier が fail した場合:
1. 初回失敗: タスクを `failed` に set し、`suggested_model` を 1 ティア上げて (haiku→sonnet、sonnet→opus) Phase 5 に再投入。
   `failure_context = { reason: verifier.reason, failed_tests: verifier が実行したテストの出力, diff: worktree の git diff }`
2. 2 回目失敗: 同様に escalate (sonnet→opus、opus は上限)。
3. 3 回目失敗: ユーザーに判断を仰ぐ (escalate 上限)。
モデルティア順: haiku < sonnet < opus。最上位 (opus) で失敗した場合は即ユーザーエスカレーション。
...
```
- FAIL の理由 (未 verified / worktree 残置 / 未コミット) を解消する。`failed` タスクは **failure_context** を構造的に注入して Phase 5 に再投入する:
  ```json
  {
    "reason": "<verifier の判定理由>",
    "failed_tests": "<verifier が実行したテストの失敗出力>",
    "diff": "<git diff HEAD~1 の出力>"
  }
  ```
  カスケードエスカレーション (Phase 6 参照) で model ティアも上げる。
  2 回目も fail したらユーザーに判断を仰ぐ。
