...
1. **タスクの status を set するとき、実際に使ったモデルとコストも一緒に書く** (escalation 後の
   ...
   # 空のときは --session を渡さず gauge の「最新セッション」既定にフォールバックする (空文字を渡すと null になる)。
   ...
   する (後方互換)。**gauge の `session --json` は v0.2.0 以降が必要** (それ未満のバイナリは `--json` を
   知らずエラー→コスト 0 になる)。なお `gauge` が返すのは**セッション累積コスト** (タスク単位ではない粗い値)
   で、単一タスク run ではタスクコストに一致し、複数タスク run では run 内で共有される。
