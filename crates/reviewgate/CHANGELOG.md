...
## [0.1.5](https://github.com/yukineko/claude-harnesses/releases/tag/reviewgate-v0.1.5) - 2026-07-02

### Added

- *(harness-status,harness-core)* Stop-gate latency observability + contract doc
- *(reviewgate)* gate project-config reviewer_cmd behind workspace trust

### Fixed

- *(reviewgate)* block instead of silently allowing a truncated diff
- *(reviewgate)* fail closed when the reviewer subprocess breaks
...
- rebuild plugin binaries [skip ci]
- *(reviewgate)* thin adapter over harness_core::gate
- rebuild plugin binaries [skip ci]
...
- Add 'crates/reviewgate/' from commit 'ce815ef3506c84aa803c4495e5d6ea01a3ca096a'
